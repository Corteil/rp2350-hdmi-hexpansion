# RP2350 HDMI Hexpansion — Design History

> **This is the project's design journal, not the current-status doc.** It's the
> full chronological record of the design rationale, bench measurements, bugs
> found/fixed, and the phase-by-phase delivery plan that got the project from a
> blank page to working hardware. For what the project actually is and does
> *today*, see [`README.md`](README.md) — come back here for the "why" behind
> any given decision, or for BOM/costing/risk-register detail the README
> doesn't repeat.
...
Status when most of this document was written: **design plan / pre-schematic**,
every figure marked *(est.)* a modelled estimate, not a quote. Much of it is now
built and working on bench hardware — see the README for current status. Left
otherwise unedited as the historical record.
